...
bool CliPacket::RecvMsg(const SockPtr sockPtr, string& msg, MsgType& type, string& receiverID)
{
	...
	if (!reader.parse(sPacket.data(), jPacket)) {
		cout << "Fail: Failed to parse data." << endl;	//err window, feedback
		return false;
	}
	...
	if (checkSum != sum) {
		cout << "Fail: Checksum is inconsistent." << endl;		//err window, feedback
		return false;
	}
...
bool CliPacket::RecvMsg(const SockPtr sockPtr, string& msg, MsgType& type)
{
	//receive data and resize the buffer
	string sPacket(300, ' ');	//try ' '
	size_t size;
...
bool CliPacket::SendMsg(const SockPtr sockPtr, const string& msg, const MsgType& type, const string& senderID)
{
	// message checksum
	char checkSum = 0;
	for (auto i : msg) {
...
bool CliPacket::SendMsg(const SockPtr sockPtr, const string& msg, const MsgType& type)
{
	// message checksum
	char checkSum = 0;
	for (auto i : msg) {
